...
adminControllers
    ...
    .controller('setCtrl', function ($scope, setFactory, $location, $anchorScroll, $timeout) {
        $scope.activeDeck = {};
    ...
    .controller('deckCtrl', function ($scope, $modal, $filter) {
        $scope.name = "deck";
        ...
        $scope.moveItem = function (id, rel) {
            if (id + rel < 0 || id + rel > $scope.deck.items.length + 1) return false;
    ...
    .controller('itemCtrl', function ($scope, $filter, itemOptions, $timeout, $anchorScroll, $location) {
        $scope.name = "item";
        $scope.anchor_id = "";
        ...
        $scope.addInlineSwitchOption = function (item) {
            ...
            if (!$scope.item.inlineDecks) $scope.item.inlineDecks = [];
        ...
        $scope.collapse = function () {
            ...
            return;
            var anchor_id = Date.now();
            $scope.anchor_id = anchor_id;
            $timeout(function () {
                $scope.scrollTo(anchor_id);
            }, 2000);
        };

        //funktioniert nicht. grrr....
