(function() {
    ...
    Poll.prototype = {
        ...
        vote: function (vote) {
            ...
            if (!(this.open && this.checkVoteValid(vote))) {
                return false;
            }
            ...
            if (typeof vote.multiplier === "undefined") {
                vote.multiplier = 1;
            }
            this.evalVote(vote);
            this.votes.push(vote);
            var requiredVotes = this.maxVotes * this.percentsForFinish / 100;
            if (this.maxVotes !== 0 && this.votes.length >= requiredVotes) {
                this.finish();
            }
            return true;
        },
        checkVoteValid: function (vote) {
            ...
            if (!ret) {
                this.log("got invalid vote");
            }
            else {
                this.log("got valid vote");
            }
            return ret;
        },
        //Stimme auswerten
